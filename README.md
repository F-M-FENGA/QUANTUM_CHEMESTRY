...
Summary
-------
This folder contains the code and notebooks associated with a Master's project in quantum chemistry. It gathers geometry optimizations, ORCA input/output handling, variational quantum algorithms for electronic structure (SSVQE), and post‑VQE analysis (QEOM).

Repository structure
--------------------
- Quantum Chemistry.ipynb
  - Main notebook (exploration, analyses, and figures). Contains cells to run experiments, show results, and produce plots.
...
- Curves_and_Plots/
  - Scripts and notebooks to generate figures for the report: energy curves, convergence plots, densities, and orbital diagrams.

Prerequisites
-------------
- Platform: Linux / macOS (Windows via WSL is possible).
- Python 3.8+ (preferably 3.9 to match examples).
...
- Typical Python packages:
  ...
  - qiskit or another quantum SDK used in the project (Pennylane)
...
2) Install Python dependencies (example):
   ```
   pip install numpy scipy pandas matplotlib seaborn jupyterlab qiskit pyscf pennylane
   ```
